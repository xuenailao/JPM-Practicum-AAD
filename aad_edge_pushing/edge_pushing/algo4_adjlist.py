"""
Algorithm 4 (Edge-Pushing) with Adjacency List optimization.

Implementation: Adjacency List
- Uses SymmSparseAdjList with O(degree) neighbor lookup
"""
...
def algo4_adjlist(output: ADVar, inputs: List[ADVar]) -> np.ndarray:
    """
    Compute Hessian using Algorithm 4 with adjacency list optimization.

    This version uses SymmSparseAdjList which maintains adjacency lists
    for O(degree) neighbor lookup, eliminating the O(n) scan bottleneck.
    ...
    W = SymmSparseAdjList(n_nodes)  # ← Uses adjacency list for fast neighbor lookup!
    vbar = [0.0] * n_nodes
    ...
    for node in reversed(global_tape.nodes):
        ...
        # PUSHING STAGE: Edge-pushing (OPTIMIZED with adjacency list!)
...
def _pushing_stage_adjlist(W: SymmSparseAdjList, i: int, preds: List[int],
    ...
    KEY OPTIMIZATION:
    Old (AdjMatrix): for p in range(n_nodes): w_pi = W.get(p, i)  # O(n) scan
    New (AdjList): neighbors = W.get_neighbors(i)                  # O(degree(i))

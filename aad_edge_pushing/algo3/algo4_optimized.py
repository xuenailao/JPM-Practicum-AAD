"""
Algorithm 4 (Edge-Pushing) with optimized SymmSparse data structure.

Key optimization:
- Uses SymmSparseOptimized with O(1) neighbor lookup
- Eliminates O(n) scan that was 67% of execution time
- Should show 2-5× speedup for sparse Hessians

Expected performance:
- Dense (n=50): Similar to Algo3
- Sparse 90% (n=100): 2-3× faster than Algo3
- Sparse 95% (n=200): 3-5× faster than Algo3
"""
...
from .symm_sparse_optimized import SymmSparseOptimized


def algo4_optimized(output: ADVar, inputs: List[ADVar]) -> np.ndarray:
    """
    Compute Hessian using Algorithm 4 with optimized sparse matrix.

    This version uses SymmSparseOptimized which maintains adjacency lists
    for O(1) neighbor lookup, eliminating the O(n) scan bottleneck.
    ...
    W = SymmSparseOptimized(n_nodes)  # ← Key change!
    vbar = [0.0] * n_nodes
    ...
    for node in reversed(global_tape.nodes):
        ...
        # PUSHING STAGE: Edge-pushing (OPTIMIZED!)
        _pushing_stage_optimized(W, i, preds, d1)
...
def _pushing_stage_optimized(W: SymmSparseOptimized, i: int, preds: List[int],
                              d1: Dict[int, float]) -> None:
    """
    OPTIMIZED pushing stage using adjacency list for neighbor lookup.

    KEY OPTIMIZATION:
    Old: for p in range(n_nodes): w_pi = W.get(p, i)  # O(n) scan
    New: neighbors = W.get_neighbors(i)                 # O(degree(i))

    For sparse matrices with degree << n, this is a huge win!

    Args:
        W: Optimized sparse matrix with adjacency list support
    ...
    # ✨ THIS IS NOW O(degree(i)) instead of O(n)!
...
def _creating_stage(W: SymmSparseOptimized, preds: List[int],
...
# ============================================================================
# Shared helper functions (reused from algo4_edge_pushing.py)
# ============================================================================
...
def _extract_input_hessian(W: SymmSparseOptimized, input_indices: List[int]) -> np.ndarray:

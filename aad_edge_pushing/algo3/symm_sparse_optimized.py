...
Key optimization for Algorithm 4:
- Maintains adjacency lists to find neighbors of a node in O(1) instead of O(n)
- Critical for sparse Hessian computation where nnz << n²

Performance gain:
- Old: O(n) scan to find neighbors → 67% of Algo4 time
- New: O(degree) lookup → Proportional to actual sparsity
"""
